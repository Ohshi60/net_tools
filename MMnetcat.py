...
def client_handler(client_socket):
	...
	if len(upload_destination):
		#read in all the bytes and write to dest
		file_buffer = ""
...
def main():
	...
	#if we are going to listen or just send data from stdin
	if not listen and len(target) and port > 0:

		#read in the buffer from the cmdline
		#this will bloc, so send CTRL-D if not sending input
		#to stdin
